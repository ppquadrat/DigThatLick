...
from dtlutil import write_rdf_with_temp as write_rdf_to_file
from dtlutil import append_rdf_with_temp as append_rdf_to_file
...
class JEtoRDF:
    ...
    def create_session(self):
        ...
        self.add( (sessionURI, RDF.type, DTL.Session) )
    ...
    def find_artist(self, artistName):
        # artist is matched on the name
        ################### This can lead to different artists with the same name to be merged
        # a famous case is Bill Evans pianist and Bill Evans sax player
        # it has been decided by the project team that these cases are very rare and we go for names as unambiguous
        for artist in self.g.subjects(predicate=FOAF.name, object=Literal(artistName)):
            if not (artist, RDF.type, MO.MusicGroup) in self.g:
                logging.debug("Artist %s found: %s", artistName, artist)
                return artist
        else: return None
    
    def exists_artist(self, artistName):
        return self.find_artist(artistName) != None
    ...
    def find_instrument(self, inst_label):
        ...
        if (None, DTL.je_inst_label, Literal(inst_label)) in self.g:
            instrument = self.g.value(subject=None, predicate=DTL.je_inst_label, object=Literal(inst_label), default=None, any=False)
            logging.debug("Instrument %s found: %s", inst_label, instrument)
            return instrument
        else:
            return None

    def exists_instrument(self, inst_label):
        return self.find_instrument(inst_label) != None

    def create_performer(self, artistURI, instrumentURI, confidence):
        ...
        self.add( (performerURI, DTL.instrument, instrumentURI) )
        # confidence 
                ...
                if row[0].startswith("CD"):
                    ...
                    for artist, instrument, confidence in musicians:
                        ...
                        try:
                            ...
                            # this was a problem in Python2, Python3 seems to handle unicode names properly
                            logging.warning("Line %i --- UnicodeDecodeError! artist name: %s", count + 1, artist)
                            artistURI = self.create_artist(artist)
                        if yes:
                            artistURI = self.find_artist(artist)
                        else:
                            artistURI = self.create_artist(artist)
                        # find or create instrument
                        if self.exists_instrument(instrument):
                            instrumentURI = self.find_instrument(instrument)
                        else:
                            instrumentURI = self.create_instrument(instrument)
                        # create performer
                        performerURI = self.create_performer(artistURI, instrumentURI, confidence)
                    ...
                    tcount = 0
                    # append the temporary graph to the rdf file, clear the temp graph
                    self.g_temp = dtlutil.append_and_clear_temp_graph(self.g_temp, rdffilename, rdffilename_tmp)
